...
- 脚本：`scripts/fill_comp_type_code.py`
...
- **脚本**：`scripts/import_finance_fields.py`（ALTER TABLE + `INSERT ... ON DUPLICATE KEY UPDATE` 幂等）
- **字段类型**：全部 `FLOAT NULL`，与既有财务字段（monetary_cap 等）一致
