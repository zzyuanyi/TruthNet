...
> **更新**: 2026-08 | 本文描述当前可运行的前端实现；早期 V12 方案仅作设计来源参考。
...
| 层级 | 技术 | 状态 |
|------|------|:---:|
| 框架 | React 18 + Vite 6 + TypeScript | ✅ 已实现 |
| UI 组件 | shadcn/ui + Tailwind CSS | ✅ 已使用 |
| 图表 | Recharts | ✅ 已用于财务趋势与指标图表 |
| 图谱 | AntV G6 Canvas | ✅ 已用于上下游、多跳股权关系图 |
| HTTP | fetch | ✅ |
...
板块: 基本信息 + 风险标签 → 财务异常（7 条规则与趋势图）→ 股权穿透图（G6，上下游与多跳路径）→ 关联方风险 → 舆情时间线 → 证据引用 → 行业对比入口
