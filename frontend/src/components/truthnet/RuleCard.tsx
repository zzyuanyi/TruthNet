...
} from "@/components/truthnet/InsightDisclosure";
...
function formatCalculationValue(value: number | string, unit?: string): string {
  ...
  if (unit === "percentage_point" || unit === "pp")
    return `${numericValue.toFixed(2)} 个百分点`;
  if (Math.abs(numericValue) >= 100_000_000)
    return `${(numericValue / 100_000_000).toFixed(2)} 亿元`;
  if (Math.abs(numericValue) >= 10_000)
    return `${(numericValue / 10_000).toFixed(2)} 万元`;
...
function calculationFieldLabel(fieldPath: string, role?: string): string {
  ...
  return (
    calculationFieldLabels[field] ||
    (roleField ? calculationFieldLabels[roleField] : undefined) ||
    roleField ||
    fieldPath
  );
}

type CalculationInputs = NonNullable<
  FinanceRuleItem["calculation_trace"]
>["inputs"];
...
function calculationDataCheckWarnings(
  ruleId: string,
  inputs: CalculationInputs,
): string[] {
  const warnings: string[] = [];
  const latestPeriod = inputs.reduce(
    (latest, input) => (input.period > latest ? input.period : latest),
    "",
  );

  // R2 的分母过小会把现金流/利润比放大，必须把原始金额和口径提示一起呈现。
  if (ruleId === "R2") {
    const cashflow = inputs.find(
      (input) =>
        input.period === latestPeriod &&
        input.field_path === "net_cash_flows_oper_act",
    );
    const profit = inputs.find(
      (input) =>
        input.period === latestPeriod &&
        input.field_path === "net_profit_excl_min_int_inc",
    );
    ...
    if (
      ...
      if (Math.abs(ratio) > 100) {
        warnings.push(
          `${formatCalculationPeriod(latestPeriod)}现金流/净利润比为 ${ratio.toFixed(1)}；净利润基数较小会放大该比值，请核对母公司报表范围与原始披露。`,
        );
      }
    }
  }
  ...
  for (const fieldInputs of byField.values()) {
    const ordered = [...fieldInputs].sort((a, b) =>
    ...
    if (relative <= 0.1) {
      warnings.push(
        `${calculationFieldLabel(latest.field_path, latest.role)}最新绝对值较前四个可比报告期中位数低 ${((1 - relative) * 100).toFixed(1)}%，建议核对期次、报表范围与原始披露。`,
      );
    } else if (relative >= 10) {
      warnings.push(
        `${calculationFieldLabel(latest.field_path, latest.role)}最新绝对值约为前四个可比报告期中位数的 ${relative.toFixed(1)} 倍，建议核对期次、报表范围与原始披露。`,
      );
    }
  }
...
function formatRuleValue(
  ...
  const digits =
    unit === "ratio"
      ? 2
      : unit === "percent" || unit === "percentage_point" || unit === "days"
        ? 1
        : 0;
...
const chartMetricConfig: Partial<
  Record<string, { seriesKey: string; benchmarkId: string }>
> = {
...
// severity → 信封卡皮肤
const disclosureSeverity: Record<RiskLevel, DisclosureSeverity> = {
  red: "critical",
  orange: "high",
  yellow: "medium",
  blue: "low",
...
export function RuleCard({
  ...
  const chartData = rule.history.map((item) => {
    ...
    if (chartConfig) {
      ...
      valueKey = Object.keys(item).find(
        (k) => k !== "period" && extractSeriesValue(item, k) !== null,
      );
  ...
  const chartMetricName =
    chartData.length > 0 && chartData[0].valueKey
  ...
  const benchmarkMetric = chartConfig
    ? rule.industry_metrics?.find(
        (m) => m.metric_id === chartConfig.benchmarkId,
      )
    : undefined;
  ...
  const calculationWarnings = rule.calculation_trace
    ...
    : [];
  const hasExtremeCurrentRatio =
    currentMetric === "cf_to_profit_ratio" &&
  ...
  return (
    ...
    >
      <CardContent className="pt-4 px-4 pb-0">
        <InsightDisclosure
          severity={disclosureSeverity[rule.severity]}
          title={
          ...
          meta={
            currentValue ? (
              <span className="hidden sm:inline-flex items-baseline gap-1 text-xs text-muted-foreground">
                <span>{"当期"}</span>
                <span
                  className={cn(
                    "font-semibold tabular-nums",
                    rule.status === "triggered" &&
                      disclosureSeverity[rule.severity] !== "info" &&
                      "text-red-600 dark:text-red-400",
                  )}
                >
                  {typeof currentValue.value === "number" &&
                  !hasExtremeCurrentRatio
                    ? currentValue.value.toFixed(2)
                    : "—"}
                  {currentValue.unit ? ` ${currentValue.unit}` : ""}
                </span>
              </span>
            ) : undefined
          }
          badge={
            <Badge
              variant="outline"
              className={cn("text-xs", riskLevelColors[rule.severity])}
            >
              <span
                className={cn(
        ...
        >
                ...
                rule.industry_metrics.map((m) => (
                  <div key={m.metric_id} className="text-xs leading-5">
                    <span className="font-medium">
                      ...
                      P75 {m.p75 != null ? m.p75.toFixed(2) : "--"} | P95{" "}
                      {m.p95 != null ? m.p95.toFixed(2) : "--"}
                    </span>
                    <span className="text-muted-foreground ml-1">
                      {m.company_percentile != null
                        ? `(分位 ${m.company_percentile.toFixed(1)}%)`
                        : ""}
                    </span>
                  </div>
                ))
          ...
          <p className="hidden">{rule.explanation}</p>
                ...
                <div className="mt-2 space-y-2 text-xs text-muted-foreground">
                  <p className="leading-5 text-foreground">
                    {calculationDescriptions[
                      rule.calculation_trace.formula_id
                    ] || "根据下列报表原始数据，按规则口径计算并与触发线比较。"}
                  </p>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {rule.calculation_trace.inputs.map((input, index) => (
                          <tr
                            key={`${input.field_path}-${input.period}-${index}`}
                          ...
                          >
                            ...
                            <td className="px-2 py-1.5">
                              {formatCalculationPeriod(input.period)}
                            </td>
                            <td className="px-2 py-1.5 text-right tabular-nums text-foreground">
                              {formatCalculationValue(input.value, input.unit)}
                            </td>
                            <td className="px-2 py-1.5">
                              {sourceTableLabels[input.source_table] ||
                                input.source_table}
                            </td>
                          </tr>
                        ))}
                  ...
                  <details open className="pt-1">
                    ...
                    <p className="mt-1 break-words font-mono text-[10px] leading-4 text-muted-foreground">
                      ...
                      {rule.calculation_trace.calculation_version &&
                        `（${rule.calculation_trace.calculation_version}）`}
                    </p>
                  </details>
            ...
            <div className="rounded-lg border border-border/60 bg-muted/20 p-2.5">
              ...
              {rule.evidence_ids.length > evidenceSummaries!.length && (
                <p className="mt-1.5 text-center text-[10px] text-muted-foreground">
                  还有 {rule.evidence_ids.length - evidenceSummaries!.length}{" "}
                  条，点击下方「查看证据」查看全部来源记录
                </p>
              )}
            </div>
          )}
