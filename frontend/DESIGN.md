...
### 股权穿透 · G6 层级图（0824 版回退，用户拍板）
- 视觉：G6 平面层级布局（上穿股东/中 标的/下穿子公司+上下游），节点按风险着色，保留拖拽/缩放。
- 交互两级：hover=轻量摘要卡（名称/类型/持股比）；click=Dialog 全量详情（类型徽章+风险等级徽章+持股+关联关系+权益链路径）。
- 上下游公司风险并入节点色与点击详情（红/橙/黄分级）。
- 已废弃：3D 恒星系（EquityOrbit3D）——用户反馈视觉混乱，回退平面图。禁止再做 3D 轨道式股权图。
