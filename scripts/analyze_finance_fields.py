...
def main() -> None:
    ...
    for table, csv_name in TABLES.items():
        ...
        fin_cols = [
            c
            for c in all_cols
            if meanings.get(c, "") and any(k in meanings[c] for k in FIN_MARKERS)
        ]
        if not fin_cols:
            continue
        # 读金融列 + wind_code
            ...
            rows.append(
                {
                    ...
                    "bank_pct": round(
                        bank[c].notna().sum() / max(len(bank), 1) * 100, 1
                    ),
    ...
    # 输出
    print(
        f"{'表':18} {'字段':32} {'含义':22} {'全量%':>6} {'银行%':>6} {'保险%':>6} {'证券%':>6} 银行n 保险n 证券n"
    )
    print("-" * 130)
    for r in rows:
        print(
            f"{r['table']:18} {r['field']:32} {r['meaning']:22} "
