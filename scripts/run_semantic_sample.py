"""Run a bounded 280-question semantic sample through the real graph."""
...
def main() -> int:
        ...
        items.append(
            {
                ...
                "annotation_source": "bounded_280_sample",
            }
        )
    print(f"[sample] selected={len(items)} sessions={len(positions)}")
    session_ids: list[str] = []
    try:
        records, session_ids = _run_with_context(items, dataset.questions)
    finally:
        _cleanup_sessions(session_ids)
    output = ROOT / "data" / "test-artifacts" / "semantic_sample_280.jsonl"
    ...
    print(f"[sample] observed={len(records)} errors={errors} report={output}")
