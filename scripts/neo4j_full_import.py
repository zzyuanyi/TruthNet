...
def infer_entity_type(name: str, name_to_category: dict) -> str:
    ...
    if not cats:
        if any(kw in str(name) for kw in ["有限公司","股份","集团","合伙","管理","投资"]):
            return "Company"
        return "Person"
...
def step_generate_entities():
    ...
    for _, row in df.iterrows():
        name_to_category[str(row["s_holder_name"])].append(row["s_holder_holdercategory"])
    ...
    print(f"  股东实体: Person={etypes.get('Person',0):,}  "
...
def step_generate_relationships():
    ...
    for idx, row in df.iterrows():
        ...
        relationships.append({
            ...
            "quantity": int(row["s_holder_quantity"]) if not pd.isna(row["s_holder_quantity"]) else None,
            "ann_dt": str(row["ann_dt"]) if not pd.isna(row["ann_dt"]) else "",
            "report_period": str(row["report_period"]) if not pd.isna(row["report_period"]) else "",
            "source_id": "shareholder_dataset",
    ...
    for r in relationships:
        ...
        if p >= 50:    pct_dist[">=50%"] += 1
        elif p >= 30:  pct_dist["30-50%"] += 1
        elif p >= 10:  pct_dist["10-30%"] += 1
        elif p >= 5:   pct_dist["5-10%"] += 1
        else:          pct_dist["<5%"] += 1
...
def step_import_entities():
    ...
    if records[0]["cnt"] > 0:
        ...
        driver.execute_query("MATCH (e:Entity {graph_version: $gv}) DETACH DELETE e", {"gv": gv})
    ...
    for bi in range(0, len(entities), BATCH):
        ...
        rows = [{k: e.get(k, "") for k in [
            ...
            "wind_code", "aliases", "match_confidence", "source_id"]} for e in batch]
        records, _, _ = driver.execute_query(cypher, {"rows": rows, "gv": gv, "now": now})
    ...
    records, _, _ = driver.execute_query(
        ...
        "RETURN e.entity_type AS t, count(e) AS cnt ORDER BY cnt DESC", {"gv": gv})
    for r in records:
        print(f"    {r['t']}: {r['cnt']:,}")
...
def step_import_relationships():
    ...
    # 清理已有
    records, _, _ = driver.execute_query(
        "MATCH ()-[r {graph_version: $gv}]->() WHERE type(r)='OWNS' RETURN count(r) AS cnt", {"gv": gv})
    if records[0]["cnt"] > 0:
        print(f" 清理已有 {records[0]['cnt']:,} 条 OWNS...")
        driver.execute_query("MATCH ()-[r:OWNS {graph_version: $gv}]->() DELETE r", {"gv": gv})
        ...
        for bi in range(0, len(relationships), BATCH):
            ...
            for r in batch:
                ...
                rows.append({
                    ...
                    "report_period": str(r.get("report_period", "")) if r.get("report_period") else "",
...
def step_import_concerted():
    ...
    for bi in range(0, len(relations), BATCH):
        ...
        rows = [{"src_id": r["source_entity_id"], "tgt_id": r["target_entity_id"],
                 "src_id2": r["source_record_id"], "conf": r["match_confidence"]} for r in batch]
        records, _, _ = driver.execute_query(cypher, {"rows": rows, "gv": gv, "now": now})
        total += records[0]["cnt"] if records else 0
    ...
    records, _, _ = driver.execute_query(
        ...
        "RETURN type(r) AS t, count(r) AS cnt ORDER BY cnt DESC", {"gv": gv})
    for r in records:
        print(f"    {r['t']}: {r['cnt']:,}")
    driver.close()
...
def step_import_fixtures():
    ...
    fixtures = [
        {
            "name": "裕兴股份",
            "entities": [
                {"entity_id": "ent_wang_jianxin", "canonical_name": "王建新", "display_name": "王建新",
                 "entity_type": "Person", "wind_code": "", "aliases": [], "match_confidence": 1.0, "source_id": "fixture"},
                {"entity_id": "ent_yuxing_holding", "canonical_name": "常州裕兴投资有限公司", "display_name": "裕兴投资",
                 "entity_type": "Company", "wind_code": "", "aliases": [], "match_confidence": 1.0, "source_id": "fixture"},
            ],
            "relationships": [
                {"src": "ent_wang_jianxin", "tgt": "ent_yuxing_holding", "pct": "85.0"},
                {"src": "ent_yuxing_holding", "tgt": "company_300305_SZ", "pct": "18.17"},
            ],
        },
        {
            "name": "贵州茅台",
            "entities": [
                {"entity_id": "ent_guizhou_sasac", "canonical_name": "贵州省国资委", "display_name": "贵州省国资委",
                 "entity_type": "Person", "wind_code": "", "aliases": [], "match_confidence": 1.0, "source_id": "fixture"},
                {"entity_id": "ent_moutai_group", "canonical_name": "中国贵州茅台酒厂(集团)有限责任公司", "display_name": "茅台集团",
                 "entity_type": "Company", "wind_code": "", "aliases": [], "match_confidence": 1.0, "source_id": "fixture"},
            ],
            "relationships": [
    ...
    for fix in fixtures:
        driver.execute_query(ent_cypher, {"rows": fix["entities"], "gv": gv, "now": now})
        rows = [{"src": r["src"], "tgt": r["tgt"], "pct": r["pct"]} for r in fix["relationships"]]
        driver.execute_query(rel_cypher, {"rows": rows, "gv": gv, "now": now})
        print(f"  {fix['name']}: {len(fix['entities'])} entities, {len(fix['relationships'])} rels")
...
def step_verify():
    ...
    # 孤立节点
    records, _, _ = driver.execute_query(
        "MATCH (e:Entity {graph_version: $gv}) WHERE NOT (e)--() RETURN count(e) AS cnt", {"gv": gv})
    print(f"  孤立实体: {records[0]['cnt']:,}")

    # 三股票验证
    VERIFY = [("600518.SH", "康美药业"), ("300305.SZ", "裕兴股份"), ("600519.SH", "贵州茅台")]
    all_pass = True
...
def main():
    ...
    parser.add_argument("--step", type=int, choices=range(1, 9), help="单独执行某一步 (1-8)")
    parser.add_argument("--from-step", type=int, choices=range(1, 9), help="从某步开始执行到结束")
