...
def _build_messages(companies: list[ComparisonAnalysisCompany]) -> list[dict]:
    ...
    system = (
        ...
        "你的任务：\n"
        "1. 输出 overall：300-400 字的整体对比分析，用 Markdown 结构化呈现，"
        "按序覆盖四个小节（每节单独一行，以 **加粗小节名** 开头）："
        "**风险画像对比**：…（各家公司风险等级/评分差异，引用具体公司名与数字）；"
        "**共同信号与差异**：…（同行业共性问题或各家特有信号）；"
        "**各自关注点**：…（按风险等级排序说明各自最需关注的风险点）；"
        "**交叉核验建议**：…（如行业共性问题下的核查方向，不荐股、不评级）；\n"
