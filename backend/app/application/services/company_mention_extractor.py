...
_SUBJECT_CLEAN_WORDS: tuple[str, ...] = _REQUEST_ACTION_WORDS + (
    ...
    "哪家",
    "哪个",
    # 8/23 修复："谁"是疑问词不是公司成分——复合比较"比亚迪和隆基谁
    # 营收更高"中右片段"隆基谁"带"谁"导致 reverse_contains 匹配失败
    # （全名"隆基绿能"非"隆基谁"子串）→ comparison_missing_peer；而
    # "比亚迪谁"因全名恰为子串侥幸命中。全局 mask 后统一拆分为
    # "比亚迪""隆基"均命中。
    "谁",
    "有哪些",
    "哪些",
    "会不会",
    "是否",
    # 8/23 疑问句式残留："有没有被处罚过"→ mask"处罚"后残留"有没有被"
    # 成垃圾 span 阻断延续（term=True 但 mention 非空走 mention 路径）。
    "有没有被",
    "有没有",
    "存在",
    "为什么",
    "简单",
    "综合",
    # 8/23 请求修饰词：省略主语的追问中"全面分析/最后总结"等——"全面"
    # "最后"会残留成垃圾 span 阻断主体延续（全面分析一下这家公司 →
    # mention"全面"not_found → 防串 entity_error）。
    "全面",
    "最后",
    "结论",
    "一个",
...
_TIME_MODIFIER_RE = re.compile(
    ...
    r"|(?:(?:19|20)\d{2})(?:年?年度报告|年?年报|年?半年报|年?中报|年?一季报|年?二季报|年?三季报|年?季报|年?财年|年(?:度|第?[一二三四]季度|第?[1-4]季度|上半年|下半年)?)(?:的)?"
...
_SUBJECT_TERMINATORS_FINANCE: tuple[str, ...] = (
    ...
    "经营活动现金流",
    "经营现金流",
    # 8/23 修复：规则名/财务术语也是终止符——"那它的存贷双高呢"
    # 中"存贷双高"不得成为疑似公司 span（与"其他应收款"同理）。
    "存贷双高",
    "存货双高",
    "双高",
    # 8/23 补齐：三表可查科目（与 indicator_semantics _METRIC_PHRASES
    # 对齐）——"那它的货币资金呢"中科目名必须是终止符，否则被当疑似
    # 公司名 → 防串阻断指代（entity_error）。科目词是封闭集合（三表
    # 字段有限），一次性补齐后不再逐个打补丁。
    "货币资金",
    "流动资产",
    "流动资产合计",
    ...
    "最新动态",
    "动态",
    # 8/23 业务追问词（省略主语的追问）——"实际控制人是谁""最近有
    # 什么负面消息吗"等在多轮中必须延续当前主体；词本身是业务谓词
    # （与"风险/异常/原因"同类），截断后走 _resolve_no_spans 延续。
    # 语义封闭集合（业务域名词），与 canonical_business_registry 的
    # _BUSINESS_QUERY_WORDS 对齐（兜底解释）。
    "实际控制人",
    "实控人",
    "控股股东",
    "第一大股东",
...
# 8/23 修复："的"加入段首剥离——"那它的其他应收款呢"剥"那它"后
# 残留段首"的"（所有格指代残留），不剥会形成垃圾 span"的其他"被
# Resolver 当疑似新公司 → 指代断裂 entity_error。前缀仅匹配段首
# 连续字符，不影响"美的/康美的"等名称内部字符（与终止符不同）。
_SINGLE_CHAR_PREFIX_RE = re.compile(r"^(?:看|查|给|它|他|她|其|都|那|的)+")
