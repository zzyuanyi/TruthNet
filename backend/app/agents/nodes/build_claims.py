...
# 规则 → 所需报表类型映射（基于 RULES_SPEC 字段-规则矩阵）
_RULE_EVIDENCE_MAP: dict[str, list[str]] = {
    "R1": ["ev_bs_01"],  # BS: acct_rcv
    "R2": ["ev_cf_01"],  # CF: net_cash_flows_oper_act
    "R3": ["ev_bs_01"],  # BS: monetary_cap, st_borrow, lt_borrow
    "R4": ["ev_bs_01"],  # BS: inventories
    "R5": ["ev_is_01"],  # IS: 各项费用
    "R6": ["ev_bs_01"],  # BS: oth_rcv
    "R7": ["ev_cf_01"],  # CF + IS 多表
}
    ...
    if results and results.finance and results.finance.rule_statuses:
        for rule_id, status in results.finance.rule_statuses.items():
            if status == "triggered":
                ...
                # 只引用实际存在的 evidence
                actual_ev_ids = [eid for eid in ev_ids if eid in evidence_index]
        ...
        if timeline:
            ...
            claims.append(
                Claim(
                    ...
                    text=f"{company_name}存在{event_count}项{cat_desc}事件，含负面信号",
                    claim_type="event",
                    severity="red",
