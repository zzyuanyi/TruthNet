...
24. **代码相似 ≠ 同一实体**：wind_code 的「后 5 位相同」/ 前缀相同都不代表同一家公司；只有 `sec_name`（证券简称）一致才可视为同一实体。
25. **重映射/合并代码前，必须先比对同期财务数值**：把新旧码同一 `report_period` 的财务值（营收/总资产等）拿出来对比——数值相同 = 重复数据（删旧码保留新码）；数值不同 = 两家不同公司（禁止合并）。⚠️ 曾仅凭「A2xxxx 后5位 = 92xxxx 后5位」把营收 34.5 亿的退市股误并入 2.17 亿的北交所公司，靠备份才回滚。
26. **A 前缀代码（如 `A04024.SZ`）= Wind 退市证券改写码**，是 `A`+5 位、无对应正式 6 位码；只能标 `delisted` 保留数据，不可「映射」回 6 位码。
27. **对库做删除/重映射等不可逆操作前，先 `mysqldump` 备份**（本仓库 `data/backups/`）。
28. 财务三表（income_statement / balance_sheet / cash_flow）有 `(wind_code, report_period, ...)` 唯一键；重映射旧码→新码时若新码已有同期数据会撞唯一键报错——这正是「旧码是重复数据」的信号，应删旧码而非重映射。
